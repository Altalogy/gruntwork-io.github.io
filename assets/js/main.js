...
/* form-serialize v0.7.1 | MIT License */
function serialize(a, b) {
  ...
  for (
    var c = b.hash ? {} : "",
      d = b.serializer || (b.hash ? hash_serializer : str_serialize),
      e = a && a.elements ? a.elements : [],
    ...
    if (
      ...
      h.name &&
      k_r_success_contrls.test(h.nodeName) &&
      !k_r_submitter.test(h.type)
    ) {
      var i = h.name,
        j = h.value;
      if (
        ...
        if (
          ("checkbox" !== h.type || h.checked || (j = ""),
          "radio" === h.type &&
            (f[h.name] || h.checked
              ? h.checked && (f[h.name] = !0)
              : (f[h.name] = !1)),
          !j && "radio" == h.type)
        )
          continue;
      } else if (!j) continue;
      if ("select-multiple" !== h.type) c = d(c, i, j);
      else {
        j = [];
        for (var k = h.options, l = !1, m = 0; m < k.length; ++m) {
          var n = k[m],
            o = b.empty && !n.value,
            p = n.value || o;
          n.selected &&
  ...
  if (b.empty) for (var i in f) f[i] || (c = d(c, i, ""));
  return c;
}
function parse_keys(a) {
  var b = [],
    c = /^([^\[\]]*)/,
    d = new RegExp(brackets),
    e = c.exec(a);
  for (e[1] && b.push(e[1]); null !== (e = d.exec(a)); ) b.push(e[1]);
  return b;
}
function hash_assign(a, b, c) {
  ...
  if ("[]" === d)
    return (
      (a = a || []),
      Array.isArray(a)
        ? a.push(hash_assign(null, b, c))
        : ((a._values = a._values || []),
          a._values.push(hash_assign(null, b, c))),
      a
    );
  if (e) {
    var f = e[1],
      g = +f;
    isNaN(g)
      ? ((a = a || {}), (a[f] = hash_assign(a[f], b, c)))
      : ((a = a || []), (a[g] = hash_assign(a[g], b, c)));
  } else a[d] = hash_assign(a[d], b, c);
  return a;
}
function hash_serializer(a, b, c) {
  if (b.match(brackets)) hash_assign(a, parse_keys(b), c);
  else {
    var f = a[b];
    f ? (Array.isArray(f) || (a[b] = [f]), a[b].push(c)) : (a[b] = c);
  }
  return a;
}
function str_serialize(a, b, c) {
  return (
    (c = c.replace(/(\r)?\n/g, "\r\n")),
    ...
    var c = function (e, t) {
      ...
      a = function (e, t, n) {
        ...
        if (("object" != typeof e && (e = null), "" === r))
          if ((e || (e = []), c(e, Array)))
            e.push(0 == t.length ? n : a(null, t.slice(0), n));
          else if (c(e, Object)) {
            for (var i = 0; null != e[i++]; );
            e[--i] = 0 == t.length ? n : a(e[i], t.slice(0), n);
          } else (e = []).push(0 == t.length ? n : a(null, t.slice(0), n));
        else if (r && r.match(/^\s*[0-9]+\s*$/)) {
          e || (e = []),
            (e[(u = parseInt(r, 10))] =
              0 == t.length ? n : a(e[u], t.slice(0), n));
        } else {
          if (!r) return n;
          var u = r.replace(/^\s*|\s*$/g, "");
          if ((e || (e = {}), c(e, Array))) {
            var s = {};
            for (i = 0; i < e.length; ++i) s[i] = e[i];
            e = s;
          }
          e[u] = 0 == t.length ? n : a(e[u], t.slice(0), n);
        }
        return e;
      },
      r = function (e) {
    ...
    return (
      (r.prototype = {
        ...
        GET: function (e) {
          ...
          for (
            var t = o(e), n = t[0], r = t[1], i = this.keys[n];
            null != i && 0 != r.length;
        ...
        SET: function (e, t) {
          var n = c(t) ? t : null,
            ...
            u = r[1],
            s = this.keys[i];
          return (this.keys[i] = a(s, u.slice(0), n)), this;
        },
        set: function (e, t) {
        ...
        COMPACT: function () {
          return (
            (this.keys = (function u(e) {
              var s = "object" == typeof e ? (c(e, Array) ? [] : {}) : e;
              "object" == typeof e &&
    ...
    (this.options = A || {}),
      ...
      (this.add = function (A) {
        ...
        for (
          ...
          if (this.hasAnchorJSLink(e[o])) u.push(o);
          else {
            if (e[o].hasAttribute("id")) n = e[o].getAttribute("id");
              ...
              for (
                c = a = this.urlify(e[o].textContent), r = 0;
                void 0 !== s && (c = a + "-" + r),
                  (r += 1),
                  -1 !== (s = i.indexOf(c));
        ...
        for (o = 0; o < u.length; o++) e.splice(u[o] - o, 1);
...
$("#modalPublicRepo").on("show.bs.modal", function (r) {
  var e = $(r.relatedTarget).data("public-repo"),
    a = $(r.relatedTarget).data("private-repo");
  $(r.currentTarget).find(".public-repo-link").attr("href", e),
    $(r.currentTarget).find(".private-repo-link").attr("href", a);
});

$("#modalPrivateRepo").on("show.bs.modal", function (r) {
  var a = $(r.relatedTarget).data("private-repo");
  $(r.currentTarget).find(".private-repo-link").attr("href", a);
});
...
$(function () {
  ...
  function calculate(numUsers) {
    ...
    switch (defaultTab) {
      case "#tab-support":
        if (numUsers <= pricing.pro_support.tier1.users_max)
          total = pricing.pro_support.tier1.price;
        else if (numUsers <= pricing.pro_support.tier2.users_max)
          total =
            pricing.pro_support.tier1.price +
            (numUsers - pricing.pro_support.tier1.users_max) *
              pricing.pro_support.tier2.price;
        else $('[data-pricing-calc="alert"]').show();
        $('[data-pricing-calc="title"]').text(
          "Your Subscription + Support Price"
        );
        break;
      default:
        if (numUsers <= pricing.subscription_only.tier1.users_max)
          total = pricing.subscription_only.tier1.price;
        else if (numUsers <= pricing.subscription_only.tier2.users_max)
          total =
            pricing.subscription_only.tier1.price +
            (numUsers - pricing.subscription_only.tier1.users_max) *
              pricing.subscription_only.tier2.price;
        else $('[data-pricing-calc="alert"]').show();
...
$(function () {
  $(".panel-title a.no-scroll").on("click", function () {
    $(".panel-title a.no-scroll")
      .not($(this))
      .find("i.fa")
      .removeClass("panel-isOpen");
      ...
      if (id.length) {
        ...
        $("html,body").animate(
          ...
          function () {
            ...
            accordion
              .parents(".panel")
              .find(".panel-title a.no-scroll i.fa")
              .addClass("panel-isOpen"); //point caret down for oepned panel
          }
        );
      } else {
        ...
        $(
          ".panel-group .panel:first-child .panel-title a.no-scroll i.fa"
        ).addClass("panel-isOpen");
      }
    }
  }
